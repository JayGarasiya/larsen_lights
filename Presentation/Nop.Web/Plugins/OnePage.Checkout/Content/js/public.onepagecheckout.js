...
var Checkout = {
  ...
  setStepResponse: function (response) {
    ...
    if (!response.goto_section && response.update_section.name == "shipping") {
...
var Billing = {
  ...
  init: function (form, saveUrl, disableBillingAddressCheckoutStep, guest, selectedStateId) {
    this.form = form;
  ...
  editAddress: function (url) {
    ...
    var form = $('#billing-address-select').closest('form');
    form.addClass("edit-address");
    $.ajax({
      cache: false,
      type: "GET",
      url: url,
      ...
      complete: function (jqXHR, textStatus) {
        $('#billing-new-address-form').show();
        $('#edit-address-button').hide();
        $('#billing-buttons-container .back-link').hide();
        $('#delete-address-button').hide();
        $('#save-address-button').show();
        var fl = new FloatingLabel(form)
        fl.moveLabelInsideFieldInputWrapper(form);
      },
      error: Checkout.ajaxFailure
    });
  },
