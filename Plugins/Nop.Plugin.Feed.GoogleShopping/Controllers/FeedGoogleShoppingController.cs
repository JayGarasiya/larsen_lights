...
public class FeedGoogleShoppingController : BaseAdminController
{
    #region Fields
    ...
    private async Task PrepareModelAsync(FeedGoogleShoppingModel model)
    {
        ...
        model.PageSizeForGoogleXmlProducts = googleShoppingSettings.PageSizeForGoogleXmlProducts;
            ...
            foreach (var file in files)
            {
                ...
                if (_nopFileProvider.FileExists(file))
                {
                    model.GeneratedFiles.Add(new GeneratedFileModel
                    {
                        ...
                        FileUrl = $"{_webHelper.GetStoreLocation(false)}files/exportimport/googleFeedXmls/{fileName}"
                    });
                }
            }
        }
    ...
    public async Task<IActionResult> Configure(FeedGoogleShoppingModel model)
    {
        ...
        googleShoppingSettings.PageSizeForGoogleXmlProducts = model.PageSizeForGoogleXmlProducts;

        await _settingService.SaveSettingAsync(googleShoppingSettings);
    ...
    public async Task<IActionResult> GenerateFeed(FeedGoogleShoppingModel model)
    {
        ...
        try
        {
            ...
            if (result)
                ...
                _notificationService.WarningNotification(await _localizationService.GetResourceAsync("Plugins.Feed.GoogleShopping.WarningResult"));
        }
        catch (Exception exc)
        {
