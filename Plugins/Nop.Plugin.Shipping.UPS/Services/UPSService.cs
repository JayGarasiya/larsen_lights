...
public class UPSService
{
    ...
    private async Task<RateRequest> CreateRateRequestAsync(GetShippingOptionRequest shippingOptionRequest, bool saturdayDelivery = false)
    {
        ...
        {
            var (shoppingCartUnitPriceWithDiscountBase, _) = await _taxService.GetProductPriceAsync(item.Product, (await _shoppingCartService.GetUnitPriceAsync(item.ShoppingCartItem, true)).unitPrice);
            var shoppingCartUnitPriceWithDiscount = await _currencyService.ConvertFromPrimaryStoreCurrencyAsync(shoppingCartUnitPriceWithDiscountBase, currency);
            var unitPrice = await _priceFormatter.FormatPriceAsync(shoppingCartUnitPriceWithDiscount);
            var unitPriceValue = shoppingCartUnitPriceWithDiscount;

            totalValue = shippingOptionRequest.Items.Sum(x => unitPriceValue * x.GetQuantity());

            //totalValue = shippingOptionRequest.Items.Sum(x => x.Product.Price * x.GetQuantity());
        ...
        request.Shipment.InvoiceLineTotal = new Shipment_InvoiceLineTotal
        {
            ...
            MonetaryValue = totalValue > 0 ? totalValue.ToString("F2", CultureInfo.InvariantCulture) : "1.00"
            //        MonetaryValue = totalValue.ToString("F2", CultureInfo.InvariantCulture)
        ...
        bool saturdayDelivery = false)
    {
        try
        {
            ...
            var upsDutyMessage = string.Empty;
            var importCountryCode = string.Empty;
            if (shippingOptionRequest?.ShippingAddress?.CountryId != 0)
            {
                var country = await _countryService.GetCountryByIdAsync(shippingOptionRequest?.ShippingAddress?.CountryId ?? 0);
                importCountryCode = country?.TwoLetterIsoCode ?? string.Empty;
            }

            var currency = await _workContext.GetWorkingCurrencyAsync();
            if (importCountryCode.Equals("US", StringComparison.OrdinalIgnoreCase) == false)
            {
                var landedCostRequestModel = new UpsLandedCostRequest
                {
                    currencyCode = currency.CurrencyCode,
                    transID = shippingOptionRequest?.Customer?.Id.ToString() ?? string.Empty,
                ...
                foreach (var cartItem in shippingOptionRequest.Items)
                {

                    var (shoppingCartUnitPriceWithDiscountBase, _) = await _taxService.GetProductPriceAsync(cartItem.Product, (await _shoppingCartService.GetUnitPriceAsync(cartItem.ShoppingCartItem, true)).unitPrice);
                    var shoppingCartUnitPriceWithDiscount = await _currencyService.ConvertFromPrimaryStoreCurrencyAsync(shoppingCartUnitPriceWithDiscountBase, currency);
                    var unitPrice = await _priceFormatter.FormatPriceAsync(shoppingCartUnitPriceWithDiscount);
                    var unitPriceValue = shoppingCartUnitPriceWithDiscount;

                    landedCostRequestModel.shipment.shipmentItems.Add(
                        new ShipmentItemRequest
                        {
                            commodityId = cartItem.ShoppingCartItem.ProductId.ToString(),
                            priceEach = unitPriceValue.ToString("0.##", CultureInfo.InvariantCulture),
                            hsCode = _upsSettings.HSCode,
                            quantity = cartItem.ShoppingCartItem.Quantity,
                            uom = _upsSettings.WeightType,
                            originCountryCode = "CN",
                            commodityCurrencyCode = "USD"
                        });
                }

                // get duty on led products
                var duty = await GetEstimatedDutyAsync(landedCostRequestModel);
                if (duty.HasValue && duty.Value > 0)
                {
                    upsDutyMessage = string.Format(
                        await _localizationService.GetResourceAsync("Plugins.Shipping.UPS.ShippingOption.Description"),
                        duty.Value.ToString("C", CultureInfo.GetCultureInfo("en-US")));
                }
            }

            // prepare shipping options
            var options = (await PrepareShippingOptionsAsync(rateResponse)).Select(shippingOption =>
                {
                    // correct option name
                    ...
                    shippingOption.Description += upsDutyMessage;
    ...
    /// Gets estimated duty
    /// </summary>
    /// <param name="model">Ups landed cost request</param>
    /// <returns>
    /// A task that represents the asynchronous operation
    /// The task result contains the represents a response of getting ups duty
    /// </returns>
    public async Task<decimal?> GetEstimatedDutyAsync(UpsLandedCostRequest model)
    {
        if (model?.shipment == null)
            return null;

        var token = await GetAccessTokenAsync();
        var baseUrl = _upsSettings.UseSandbox ? "https://wwwcie.ups.com" : "https://onlinetools.ups.com";

        using var request = new HttpRequestMessage(HttpMethod.Post, $"{baseUrl}/api/landedcost/v1/quotes");
        request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", token);
        request.Headers.Add("transId", string.IsNullOrEmpty(model.transID) ? Guid.NewGuid().ToString("N") : model.transID);
        request.Headers.Add("transactionSrc", "LED-lights");
        request.Headers.Add("AccountNumber", _upsSettings.AccountNumber);
        ...
        {
            await _logger.ErrorAsync($"UPS Estimated Duty error: {responseBody}");
            return null;
        }

        var result = JsonConvert.DeserializeObject<UpsLandedCostResponse>(responseBody);
        return result?.shipment?.grandTotal;
    }
