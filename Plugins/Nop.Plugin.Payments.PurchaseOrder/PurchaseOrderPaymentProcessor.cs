...
namespace Nop.Plugin.Payments.PurchaseOrder
{
    ...
    public class PurchaseOrderPaymentProcessor : BasePlugin, IPaymentMethod
    {
        ...
        public async Task<bool> HidePaymentMethodAsync(IList<ShoppingCartItem> cart)
        {
            ...
            //hide this payment method if not allow to regular login or by customer roles
            var customer = await _customerService.GetCustomerByIdAsync(cart.FirstOrDefault().CustomerId);
            var limitedtoroles = _purchaseOrderPaymentSettings.SelectedCustomerRoleIds is null ? true : _purchaseOrderPaymentSettings.SelectedCustomerRoleIds.Any() ?
                _purchaseOrderPaymentSettings.SelectedCustomerRoleIds.Intersect(await _customerService.GetCustomerRoleIdsAsync(customer)).Any() : true;
            
            var showImpersonated = _purchaseOrderPaymentSettings.ShowImpersonated ? _workContext.OriginalCustomerIfImpersonated != null : true;

            if (!limitedtoroles)
                return _purchaseOrderPaymentSettings.ShowImpersonated ? !showImpersonated : !limitedtoroles;
            
            if (!showImpersonated)
                return limitedtoroles ? !limitedtoroles : !showImpersonated;
        ...
        {
            var purchaseOrderNumber = form["PurchaseOrderNumber"].ToString() ?? string.Empty;
            string purchaseOrderNumberResource = await _localizationService.GetResourceAsync("Plugins.Payment.PurchaseOrder.PurchaseOrderNumber");

            var paymentRequest = new ProcessPaymentRequest();
            {
                paymentRequest.CustomValues.Add(new(purchaseOrderNumberResource, purchaseOrderNumber.ToString(), displayToCustomer: true));
            }
        ...
        public override async Task InstallAsync()
        {
            ...
            await _localizationService.AddOrUpdateLocaleResourceAsync(new Dictionary<string, string>
            {
                ...
                ["Plugins.Payment.PurchaseOrder.ShowImpersonated"] = "Show for impersonated customer",
                ["Plugins.Payment.PurchaseOrder.ShowImpersonated.Hint"] = "An option indicating whether show for impersonated customer are required in order to display this payment method during checkout.",
                ["Plugins.Payment.PurchaseOrder.LimitedToRoles"] = "Limited to customer roles",
                ["Plugins.Payment.PurchaseOrder.LimitedToRoles.Hint"] = "Select customer roles for which the payment method during purchaseOrder will be shown. Leave empty if you want this to be visible to all users.",
