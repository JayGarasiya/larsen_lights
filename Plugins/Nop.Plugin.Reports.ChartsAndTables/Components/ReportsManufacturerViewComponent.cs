...
namespace Nop.Plugin.Reports.ChartsAndTables.Components
{
    ...
    /// </summary>
    public class ReportsManufacturerViewComponent : NopViewComponent
    {
        #region Fields
        ...
        public ReportsManufacturerViewComponent(ChartsAndTablesSettings chartsAndTablesSettings,
        ...
        public async Task<IViewComponentResult> InvokeAsync(string widgetZone, object additionalData)
        {
            ...
            if (!await _permissionService.AuthorizeAsync(ReportsPermissionProvider.ACCESS_ADMIN_ACCESS_REPORTS) &&
                ...
                !await _permissionService.AuthorizeAsync(StandardPermission.Catalog.MANUFACTURER_CREATE_EDIT_DELETE))
                return Content(string.Empty);

            var baseModel = additionalData as ManufacturerModel;
            if (baseModel.Id == 0)
                return Content(string.Empty);

            if (!_chartsAndTablesSettings.ManufacturerReport)
                return Content(string.Empty);

            var model = await _chartsAndTablesModelFactory.PrepareReportDetailManufacturerSearchModelAsync(new ChartsAndTablesSearchModel());
            model.ManufacturerId = baseModel.Id;

            return View("~/Plugins/Reports.ChartsAndTables/Views/Manufacturer/ManufacturerDetailReport.cshtml", model);
        }
