...
namespace Nop.Plugin.Reports.ChartsAndTables.Components
{
    ...
    /// </summary>
    public class ReportsProductViewComponent : NopViewComponent
    {
        #region Fields
        ...
        public ReportsProductViewComponent(ChartsAndTablesSettings chartsAndTablesSettings,
        ...
        public async Task<IViewComponentResult> InvokeAsync(string widgetZone, object additionalData)
        {
            ...
            var baseModel = additionalData as ProductModel;
            if(baseModel.Id == 0)
                return Content(string.Empty);

            if (!_chartsAndTablesSettings.ProductReport)
                return Content(string.Empty);

            var searchModel = new ChartsAndTablesSearchModel
            {
                ProductId = baseModel.Id
            };

            var model = await _chartsAndTablesModelFactory
                .PrepareReportDetailProductSearchModelAsync(searchModel);


            return View("~/Plugins/Reports.ChartsAndTables/Views/Product/ProductDetailReport.cshtml", model);
        }
