...
namespace Nop.Plugin.Reports.ChartsAndTables.Components
{
    ...
    /// </summary>
    public class ReportsVendorViewComponent : NopViewComponent
    {
        #region Fields
        ...
        public ReportsVendorViewComponent(ChartsAndTablesSettings chartsAndTablesSettings,
        ...
        public async Task<IViewComponentResult> InvokeAsync(string widgetZone, object additionalData)
        {
            ...
            if (!widgetZone.Equals(AdminWidgetZones.VendorDetailsBlock))
                return Content(string.Empty);
            ...
            if (!await _permissionService.AuthorizeAsync(ReportsPermissionProvider.ACCESS_ADMIN_ACCESS_REPORTS) &&
                ...
                !await _permissionService.AuthorizeAsync(StandardPermission.Customers.VENDORS_CREATE_EDIT_DELETE))
                return Content(string.Empty);

            var baseModel = additionalData as VendorModel;
            if (baseModel.Id == 0)
                return Content(string.Empty);

            if (!_chartsAndTablesSettings.VendorReport)
                return Content(string.Empty);

            var model = await _chartsAndTablesModelFactory.PrepareReportDetailVendorSearchModelAsync(new ChartsAndTablesSearchModel());
            model.VendorId = baseModel.Id;

            return View("~/Plugins/Reports.ChartsAndTables/Views/Vendor/VendorDetailReport.cshtml", model);
        }
