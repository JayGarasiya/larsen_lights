...
public class CustomerRoleDiscountRequirementRule : BasePlugin, IDiscountRequirementRule
{
    ...
    public async Task<DiscountRequirementValidationResult> CheckRequirementAsync(DiscountRequirementValidationRequest request)
    {
        ...
        var restricted = (await _customerService.GetCustomerRolesAsync(request.Customer)).Any(role => role.Id == restrictedRoleId);
        ...
        var dealer = (await _customerService.GetCustomerRolesAsync(request.Customer)).Any(role => dealerRoleIds.Any(dr => dr == role.Id));

        if (!dealer)
            result.IsValid = restricted;
        else
        {
            var currentPriceId = await _genericAttributeService.GetAttributeAsync<int>(request.Customer, "DealerPrice", request.Store.Id, 1);
            if (currentPriceId == 2)            //[1= Dealer price, 2= List price]
                result.IsValid = restricted && !dealerRoleIds.Any(dr => dr == restrictedRoleId);
            else
                result.IsValid = dealer && restricted;
        }
